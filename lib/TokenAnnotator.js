...
var TokenAnnotator = function (props) {
    ...
    var handleMouseUp = function () {
        ...
        props.onChange(__spreadArray(__spreadArray([], props.value, true), [getSpan({ start: start, end: end, tokens: props.tokens.slice(start, end) })], false));
        window.getSelection().empty();
    };
    var handleSplitClick = function (_a) {
        ...
        if (splitIndex >= 0) {
            props.onChange(__spreadArray(__spreadArray([], props.value.slice(0, splitIndex), true), props.value.slice(splitIndex + 1), true));
        }
    };
    var tokens = props.tokens, value = props.value, onChange = props.onChange, _ = props.getSpan, _1 = props.rendermark, divProps = __rest(props, ["tokens", "value", "onChange", "getSpan", "rendermark"]);
    var splits = (0, utils_1.splitTokensWithOffsets)(tokens, value);
    return (react_1["default"].createElement("div", __assign({}, divProps, { onMouseUp: handleMouseUp }), splits.map(function (split, i) {
        return split.mark ? (rendermark(__assign({ key: "".concat(split.start, "-").concat(split.end) }, split))) : (react_1["default"].createElement(Token, __assign({ key: split.i }, split)));
    })));
};
